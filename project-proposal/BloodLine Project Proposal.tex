...
\newcommand{\sectsub}[2]{\section{#1\hfill\normalfont\normalsize\itshape\ldots #2}}
\newcommand{\subsectsub}[2]{\subsection{#1\hfill\normalfont\normalsize\itshape\ldots #2}}
...
\author{
Avraj Singh Sekhon, CSED\thanks{Roll No: \texttt{1024030287}, Email: \texttt{asekhon\_be24thapar.edu}} \and
Krish Kumar, CSED\thanks{Roll No: \texttt{1024030296}, Email: \texttt{kkumar3\_be24thapar.edu}} \and
Kyna Sood, CSED\thanks{Roll No: \texttt{1024030303}, Email: \texttt{ksood\_be24thapar.edu}} \and
Manya Sabherwal, CSED\thanks{Roll No: \texttt{1024030288}, Email: \texttt{msabherwal\_be24thapar.edu}}
}
...
\textbf{Why this matters now (contextual relevance):} A proactive, data-driven coordination layer can help blood-bank staff prepare for potential shortages earlier and reduce avoidable delays in donor outreach or cross-bank coordination.

\sectsub{Proposed Solution}{Software Proposition}
\subsection{Overview}
Build a \textbf{web-based} ``BloodLine'' system with the following components:
\begin{itemize}
...
\sectsub{Solution Approach}{Engineering focus}
...
\subsectsub{Forecasting and donor-matching assistance}{non-research}
...
\subsectsub{Web architecture}{web-based solution}
...
\sectsub{Evaluation Criterion}{measurable and attributable}
...
\item \textbf{Baseline comparison:} Prophet/SARIMA results will be compared against a simple moving-average or naive forecast baseline on the same test period, so that MAE and RMSE improvements are demonstrably attributable to the modelling approach rather than to the characteristics of the simulated data.
\end{itemize}
...
\subsectsub{Pilot validation plan}{high-level}
\begin{itemize}
\item Use a simulated demand dataset and divide it chronologically into training and testing periods.
\item Compare metrics before and after within the iteration window using forecast outputs and seeded donor and blood-bank scenarios.
\end{itemize}

\sectsub{Scalability}{with foresight}
...
\sectsub{Project scope and deliverables}{iteration-friendly}
\subsectsub{Initial deliverable}{first iteration}
\begin{itemize}
